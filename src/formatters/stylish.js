...
const formatNodeValue = (value, depth) => {
  if (_.isObject(value)) {
    const formatEntry = ([key, val]) => `${getIndent(depth)}${key}: ${formatNodeValue(val, depth + 1)}`;
...
const createChange = (name, value, sign, depth) => `${getIndent(depth, true)}${sign} ${name}: ${formatNodeValue(value, depth + 1)}`;
...
const createUnchanged = (name, value, depth) => `${getIndent(depth)}${name}: ${formatNodeValue(value, depth + 1)}`;

const makeStylish = (abstractTree) => {
  const buildStylishedTree = (tree, depth) => {
    const indent = getIndent(depth);
    const stylishedTree = tree.map((node) => {
      const {
